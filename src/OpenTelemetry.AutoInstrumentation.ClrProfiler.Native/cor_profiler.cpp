...
std::vector<WSTRING> GetInterfacesInType(const ComPtr<IMetaDataImport2>& metadata_import, const mdTypeDef& typeDef) {
  std::vector<WSTRING> results;
  mdToken interfaceToken;
  WCHAR interfaceNameBuffer[255];
  ...
  while (enumIterator != enumTypes.end()) {
      ...
      if (hr == S_OK) {
        ...
        if (hr == S_OK) {
          results.push_back(interfaceNameBuffer);
        }
      }
      enumIterator = ++enumIterator;
  }
...
HRESULT STDMETHODCALLTYPE CorProfiler::ModuleLoadFinished(ModuleID module_id,
  ...
  if (module_info.assembly.name != WStr("Microsoft.AspNetCore.Hosting") &&
      ...
      WCHAR typeDefNameBuffer[255];   // Stores class name
      WCHAR parentClassNameBuffer[255] = WStr("");       // Stores parent class name
      ...
      if (valid_integrations.size() == 0) {
        Debug("ModuleLoadFinished: No valid integrations found in for module ",
...
#ifdef _WIN32
  WSTRING native_profiler_file = WStr("OPENTELEMETRY.AUTOINSTRUMENTATION.CLRPROFILER.NATIVE.DLL");
#else // _WIN32
...
size_t CorProfiler::CallTarget_RequestRejitForModule(
  ...
  for (const ValidIntegration& integration : valid_integrations) {
    if (integration.wrapperMethod.action != WStr("DEFAULT") &&
        integration.wrapperMethod.action != calltarget_modification_action)
      continue;
    ...
    if (theWrapper.assembly == WStr("DEFAULT")) {
      // std::cout << "switching to default\n";
      theWrapper.assembly =
          WStr("Inception.ClrProfiler.Managed, Version=1.0.0.0, Culture=neutral, PublicKeyToken=d1cede69117c04c0");
      theWrapper.type =
          WStr("Inception.ClrProfiler.AutoInstrumentation.Custom.CustomIntegrations");
      theWrapper.action = calltarget_modification_action;
    }

    MethodReference caller_method;
    MethodReference target_method;
    MethodReference wrapper_method(theWrapper.assembly, theWrapper.type,
                                   WStr(""), theWrapper.action, Version(),
                                   Version(), {},
                                   {});
